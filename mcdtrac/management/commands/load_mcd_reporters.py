...
#            _role = 'Other Health Workers'
            _role = d[self.order['title']].strip()
...
#            print _name, _phone, _role
            ...
            role = find_closest_match(_role, Group.objects.all())
            msisdn, backend = assign_backend(_phone)
            conn = None
            try:
                conn = Connection.objects.get(identity=msisdn)
                count +=1
            except Connection.DoesNotExist:
                pass
#            print facility, role, conn, count
...
#            facility = find_closest_match(_fac, HealthFacility.objects.all(), True)
            ...
            if role in Group.objects.all():
                contact.groups.add(role)
            else:
                contact.groups.add(Group.objects.get(name='HC'))
